...
bitcoin-cli.exe -regtest createwallet "btrustwallet"
...
# WRITE YOUR SOLUTION BELOW:
bitcoin-cli.exe -regtest createwallet "treasurewallet"

# Generate an address for mining in the treasure wallet
# STUDENT TASK: Generate a new address in the treasurewallet
# WRITE YOUR SOLUTION BELOW:
TREASURE_ADDR=$(bitcoin-cli.exe -regtest -rpcwallet=treasurewallet getnewaddress)
...
# WRITE YOUR SOLUTION BELOW:
BALANCE=$(bitcoin-cli.exe -regtest -rpcwallet=btrustwallet getbalance)
...
# WRITE YOUR SOLUTION BELOW:
LEGACY_ADDR=$(bitcoin-cli.exe -regtest -rpcwallet=treasurewallet getnewaddress "" legacy)
check_cmd "Legacy address generation"

P2SH_ADDR=$(bitcoin-cli.exe -regtest -rpcwallet=treasurewallet getnewaddress "" p2sh-segwit)
check_cmd "P2SH address generation"

SEGWIT_ADDR=$(bitcoin-cli.exe -regtest -rpcwallet=treasurewallet getnewaddress "" bech32)
check_cmd "SegWit address generation"

TAPROOT_ADDR=$(bitcoin-cli.exe -regtest -rpcwallet=treasurewallet getnewaddress "" bech32m)
...
# WRITE YOUR SOLUTION BELOW:
NEW_BALANCE=$(bitcoin-cli.exe -regtest -rpcwallet=treasurewallet getbalance)
...
# WRITE YOUR SOLUTION BELOW:
P2SH_VALID=$(bitcoin-cli.exe -regtest validateaddress "$P2SH_ADDR" | jq -r '.isvalid')
...
SIGNATURE=$(bitcoin-cli.exe -regtest -rpcwallet=treasurewallet signmessage $LEGACY_ADDR "$SECRET_MESSAGE")
...
# WRITE YOUR SOLUTION BELOW:
VERIFY_RESULT=$(bitcoin-cli.exe -regtest verifymessage "$LEGACY_ADDR" "$SIGNATURE" "$SECRET_MESSAGE")
...
# WRITE YOUR SOLUTION BELOW:
NEW_TAPROOT_ADDR=$(bitcoin-cli.exe -regtest -rpcwallet=btrustwallet getnewaddress "" bech32m)
...
# WRITE YOUR SOLUTION BELOW:
ADDR_INFO=$(bitcoin-cli.exe -regtest getaddressinfo "$NEW_TAPROOT_ADDR")
...
# WRITE YOUR SOLUTION BELOW:
TAPROOT_DESCRIPTOR=$(bitcoin-cli.exe -regtest getdescriptorinfo "$SIMPLE_DESCRIPTOR" | jq -r '.descriptor')
...
# WRITE YOUR SOLUTION BELOW:
DERIVED_ADDR_RAW=$(bitcoin-cli.exe -regtest deriveaddresses "$TAPROOT_DESCRIPTOR" | jq -r '.[0]')
